<script lang="ts">
  ...
  import { createMutationState } from "$lib/createMutationState";
  import type { RouterInput, RouterOutput } from "$lib/routers";
  import type { MutationState } from "@tanstack/svelte-query";
  ...
  const mutationState = createMutationState<
    MutationState<RouterOutput["addEvent"], unknown, RouterInput["addEvent"], any>
  >({
    filters: {
  ...
  const { form, errors, data, setData, setTouched, setErrors } = createForm({
    initialValues: {
      title: "",
      scheduledDate: "",
      scheduledTime: "",
  ...
  $: if ($mutationState.length) {
    ...
    // set data
    const formData = lastMutationState.variables;
    if (formData) setData(formData);

    // set errors
    const fieldErrors = lastMutationState.context?.fieldErrors;
    if (fieldErrors) {
...
<form
  ...
  {#if $data.attendees?.length}
    {#each $data.attendees as aud, idx}
            ...
            <Tooltip.Root>
              <Tooltip.Trigger asChild let:builder>
                <button
                  ...
                  class="ml-auto bg-destructive text-destructive-foreground size-10 rounded-lg flex items-center justify-center
              focus:ring-2 focus:ring-destructive focus:ring-offset-2 focus:ring-offset-background">
                  <div class="icon-[heroicons--x-circle] size-6 me-0.5" />
                </button>
              </Tooltip.Trigger>
              <Tooltip.Content
                align="end"
